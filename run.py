...
###add by qfq
qfq_dir = "qfqtest_" + basefname[0:basefname.index('_')] + '_' + FLAGS.domain_A + '-' + FLAGS.domain_B
if not os.path.exists(qfq_dir):
    os.mkdir(qfq_dir)
###add by qfq
    ...
    with tf.Graph().as_default():
        ...
        def eval_one_epoch(epoch_num, mustSavePly=False):
            ...
            for j in range(num_batch):
                ...
                Predicted_B_xyz = np.squeeze(np.array(results["Predicted_B"]))
                feed_dict = {
                    model.pointSet_A_ph: Predicted_A_xyz,
                    model.pointSet_B_ph: Predicted_B_xyz,
                    model.is_training_ph: is_training,
                }
                results = sess.run(fetches, feed_dict=feed_dict)
                ...
                if epoch_num  % 20 == 0  or  mustSavePly:
                    ...
                    nametosave = names[begidx: endidx, ...]
                    PPredicted_A_xyz = np.squeeze(np.array(results["Predicted_A"]))
                    PPredicted_B_xyz = np.squeeze(np.array(results["Predicted_B"]))
                    ioUtil.output_point_cloud_ply(PPredicted_A_xyz, nametosave, qfq_dir,
                                                    'Ep' + str(epoch_num) + '_ppredicted_' + FLAGS.domain_A  + 'X1')
                    ioUtil.output_point_cloud_ply(PPredicted_B_xyz, nametosave, qfq_dir,
                                                    'Ep' + str(epoch_num) + '_ppredicted_' + FLAGS.domain_B  + 'X1')

                    for i in range(3):
                       results = sess.run(fetches, feed_dict=feed_dict)
                       PPredicted_A_xyz__ = np.squeeze(np.array(results["Predicted_A"]))
                       PPredicted_B_xyz__ = np.squeeze(np.array(results["Predicted_B"]))
                       PPredicted_A_xyz = np.concatenate((PPredicted_A_xyz, PPredicted_A_xyz__), axis=1)
                       PPredicted_B_xyz = np.concatenate((PPredicted_B_xyz, PPredicted_B_xyz__), axis=1)
